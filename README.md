...
| `k2kmessages.py` | Matches a mirrored screen line against the OS ROM's own message strings, so the tool can say *what* a dialog is and not just draw it. **The ~7,500-string pool is not embedded** — it has no law behind it, so a dump of it here would be a copy of part of the OS ROM and nothing else; the extractor reads it from an image the user already has. The exception is `STATE_MARKERS`: a few dozen short functional phrases ("please wait", "are you sure") that the mirror is unsafe without and that the project has shipped since its first release, now sourced from the ROM pool rather than collected by hand, and held to it by a test | GPL-2.0-or-later | Original code |
